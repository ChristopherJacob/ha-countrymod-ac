...
| Temperature, Celsius | UI permits every integer `16..30` when `temperature_unit=0`; it sends the displayed integer as `V`, `C=03`. For `T` in `16..30`: `5A 5A 06 01 03 TT ((BD+TT) mod 256) 0D 0A`. | `yt` `TemperatureCtrl` (`s=16`, `c=30`, function `r` and setter `w`); `Xt._.Temperature`; `Ft=3`. |
| Temperature, Fahrenheit | UI permits every integer `61..86` when `temperature_unit=1`, sent directly as `V`, `C=03`; the same frame formula applies. Unit selection sends `C=20,V=01` for Celsius or `C=20,V=02` for Fahrenheit. | `yt` `TemperatureCtrl`; `Xt._.Temperature`; `Xt._.SettingTemperatureUnit`; `zt=32`. |
